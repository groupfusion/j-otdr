...
public class OTDRAnalysisTest {
    ...
    public static void read(byte[] content) {
        ...
        System.out.println("Map Version : " + readInt(content, offset, LENGTH_SHORT)*0.01);
        ...
        offset += LENGTH_LONG;
        System.out.println("Map Block Count : " + (readInt(content, offset, LENGTH_SHORT)-1));
...
//        System.out.println("Map Block Content : " + Arrays.toString(mapContent));
        ...
        while (offset < mapLength) {
            ...
            System.out.println("Version : " + readInt(content, offset, LENGTH_SHORT)*0.01);
            ...
            switch (blockId) {
                case "GenParams":
                    if(format==2) {
                        printGenParams2(content, contentOffset);
                    }else{
                        printGenParams1(content, contentOffset);
                    }
//                    printGenParams(content, contentOffset);
    ...
    private static void printGenParams1(byte[] content, int offset) {
        String [] fields = {
                "cable ID",  // ........... 0
                "fiber ID",  // ........... 1
        ...
        System.out.println("Language : " + readString(content, offset, 2));
        offset += 2;
        int count = 0;
        String xstr;
        for(String field :fields){
            ...
            if (field == "build condition") {
                xstr = readString(content, offset, 2);
                offset += 2;
                System.out.println(field+ " : " + xstr);
            }else if(field.equals("wavelength")){
                int xint = readInt(content, offset, 2);
                offset += 2;
                System.out.println(field+" : " + xint);
            }else if( field.equals("user offset")){
                int xint = readInt(content, offset, 4);
                offset += 4;
                System.out.println(field+" : " + xint);
            }else{
                xstr = readStringSpaceZero(content, offset);
    ...
    private static void printGenParams2(byte[] content, int offset) {
        String [] fields = {
                "cable ID",  // ........... 0
                "fiber ID",  // ........... 1
    ...
    private static void printGenParams(byte[] content, int offset) {
        ...
        offset += name.getBytes().length + 1;
        String language=readString(content, offset, 2);
